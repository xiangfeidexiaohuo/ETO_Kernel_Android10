					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	if (cs35l41->pdata.spk_id_gpio_p)
		ucontrol->value.integer.value[0] = spk_id_get1(cs35l41->pdata.spk_id_gpio_p);
...
int spk_id_get1(struct device_node *np)
{
	int id;
	int state;
				...
				struct cs35l41_platform_data *pdata)
{
	...
	if (!pdata->spk_id_gpio_p) {
		...
		pdata->mnSpkType = spk_id_get1(pdata->spk_id_gpio_p);
	}
